...
class FastText(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_classes, word_embeddings=None):
        ...
        if word_embeddings is not None:
            self.embeddings = self.embeddings.from_pretrained(word_embeddings, freeze=False, padding_idx=0)
    ...
    def forward(self, x):
        ...
        id_embed = self.embeddings(x)
        aggregation = id_embed.sum(dim=1)
        # aggregation = id_embed.max(1)[0]
        y = self.W(aggregation)
        return y
