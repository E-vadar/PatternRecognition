...
class TextDS:
    def __init__(self, path, preprocessor=Preprocessor(), num_terms=10000, batch_size=__settings__.batch_size, split=__settings__.split, dev='cuda'):
        """
    ...
    def batch_method(self, batch):
        ...
        for i in range(len(sentences)):
            review_length = len(sentences[i])
            for j in range(review_length-2):
                y.append(sentences[i][j+1])
                x.append([sentences[i][j],sentences[i][j+2]])
        return x, y
